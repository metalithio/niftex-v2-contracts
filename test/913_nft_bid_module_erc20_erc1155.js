...
contract('Workflow', function (accounts) {
	...
	describe('Initialize', function () {
		...
		after(async function () {
			...
			assert.equal(await this.mocks.erc20.balanceOf(instance.address),       web3.utils.toWei('0'));
		});
	});
	...
	describe('Prepare tokens', function () {
		it('perform', async function () {
			...
			await this.mocks.erc20.mint(other1, web3.utils.toWei('10'));
			await this.mocks.erc20.mint(other2, web3.utils.toWei('10'));
		});
		...
		after(async function () {
			...
			assert.equal(await instance.balanceOf(other2),                  '0');
			assert.equal(await instance.balanceOf(other3),                  '0');
			assert.equal(await this.mocks.erc1155.balanceOf(instance.address,1),'1');
			assert.equal(await this.mocks.erc20.balanceOf(instance.address),       web3.utils.toWei('0'));
		});
	});
	...
	describe('Workflow NftBidModule ERC20 - ERC1155', function () {
		describe('other1 and other2 approve ERC20 transfer for nftBidModule', function () {
			it('perform', async function() {
				await this.mocks.erc20.approve(this.modules.nftbid.address, constants.MAX_UINT256, { from: other1 });
				await this.mocks.erc20.approve(this.modules.nftbid.address, constants.MAX_UINT256, { from: other2 });
			})
		});

		describe('other1 bids 5 $MOCK', function () {
			it('perform', async function () {
				const { receipt } = await this.modules.nftbid.bidWithERC20(
					this.mocks.erc1155.address,
					1,
					this.mocks.erc20.address,
					web3.utils.toWei('5'),
					{
						from: other1,
					}
			...
			after(async function () {
				...
				assert.equal(await this.mocks.erc1155.balanceOf(instance.address,1),'1');
				assert.equal(await this.mocks.erc20.balanceOf(instance.address),       		web3.utils.toWei('0'));
				assert.equal(await this.mocks.erc20.balanceOf(this.modules.nftbid.address),web3.utils.toWei('5'));
			});
		});

		describe('other1 withdraws bid', function () {
			it('perform', async function () {
				const { receipt } = await this.modules.nftbid.withdrawBidERC20(
					this.mocks.erc1155.address,
					1,
					this.mocks.erc20.address,
					{
			...
			after(async function () {
				...
				assert.equal(await this.mocks.erc1155.balanceOf(instance.address,1),'1');
				assert.equal(await this.mocks.erc20.balanceOf(instance.address),       		web3.utils.toWei('0'));
				assert.equal(await this.mocks.erc20.balanceOf(this.modules.nftbid.address),web3.utils.toWei('0'));
			});
		});

		describe('other1 bids 1 $MOCK', function () {
			it('perform', async function () {
				const { receipt } = await this.modules.nftbid.bidWithERC20(
					this.mocks.erc1155.address,
					1,
			...
			after(async function () {
				...
				assert.equal(await this.mocks.erc1155.balanceOf(instance.address,1),'1');
				assert.equal(await this.mocks.erc20.balanceOf(instance.address),       		web3.utils.toWei('0'));
				assert.equal(await this.mocks.erc20.balanceOf(this.modules.nftbid.address),web3.utils.toWei('1'));
			});
		});

		describe('other2 bids 10 $MOCK', function () {
			let diffOther1Bal = '0';
			it('perform', async function () {
				const other1PrevBal = await this.mocks.erc20.balanceOf(other1);
				const { receipt } = await this.modules.nftbid.bidWithERC20(
					this.mocks.erc1155.address,
					1,
					this.mocks.erc20.address,
				...
				const other1Bal = await this.mocks.erc20.balanceOf(other1);
			...
			after(async function () {
				...
				assert.equal(await this.mocks.erc1155.balanceOf(instance.address,1),'1');
				assert.equal(await this.mocks.erc20.balanceOf(instance.address),       		web3.utils.toWei('0'));
				assert.equal(await this.mocks.erc20.balanceOf(this.modules.nftbid.address),web3.utils.toWei('10'));
		...
		describe('compile setApprovalForAll data for ActionModule', function() {
			it('perform', async function() {
				const to = this.mocks.erc1155.address;
				const value = web3.utils.toWei('0');
				const data = this.mocks.erc1155.contract.methods.setApprovalForAll(this.modules.nftbid.address, true).encodeABI();
		...
		describe('Schedule action - acceptOfferERC1155 with invalid deadline', function () {
			it('perform', async function () {
				offerDeadline   = Math.round(new Date().valueOf()/1000) - 10000;
				const to    = this.modules.nftbid.address;
				const value = web3.utils.toWei('0');
				const data  = this.modules.nftbid.contract.methods.acceptERC1155(
					this.mocks.erc1155.address,
					1,'0x',
					this.mocks.erc20.address,
					web3.utils.toWei('10'),
		...
		describe('Wait action - acceptOfferERC1155 with invalid deadline', function () {
			it('perform', async function () {
		...
		describe('Execute action - acceptOfferERC1155 with invalid deadline', function () {
			it('perform', async function () {
				const to    = this.modules.nftbid.address;
				const value = web3.utils.toWei('0');
				const data  = this.modules.nftbid.contract.methods.acceptERC1155(
		...
		describe('Schedule action - acceptOfferERC1155 with min amount higher than highest bid', function () {
			it('perform', async function () {
		...
		describe('Wait action - acceptOfferERC1155 with min amount higher than highest bid', function () {
			it('perform', async function () {
				await web3.currentProvider.send({ jsonrpc: '2.0', method: 'evm_increaseTime', params: [ Number(deadline) - (await web3.eth.getBlock('latest')).timestamp ], id: 0 }, () => {});
			});
		...
		describe('Execute action - acceptOfferERC1155 with min amount higher than highest bid', function () {
			it('perform', async function () {
				const to    = this.modules.nftbid.address;
				const value = web3.utils.toWei('0');
				const data  = this.modules.nftbid.contract.methods.acceptERC1155(
		...
		describe('Schedule action - acceptOfferERC1155 with right min amount and deadline', function () {
			it('perform', async function () {
				offerDeadline = (await web3.eth.getBlock('latest')).timestamp + 100000;
				const to    = this.modules.nftbid.address;
				const value = web3.utils.toWei('0');
		...
		describe('Wait action - acceptOfferERC1155 with right min amount and deadline', function () {
			it('perform', async function () {
				await web3.currentProvider.send({ jsonrpc: '2.0', method: 'evm_increaseTime', params: [ Number(deadline) - (await web3.eth.getBlock('latest')).timestamp ], id: 0 }, () => {});
			});
		...
		describe('Execute action - acceptOfferERC1155 with right min amount and deadline', function () {
			let diffAdmin = '0';
			let diffArtist = '0';
			it('perform', async function () {
				const adminBalBefore = await this.mocks.erc20.balanceOf(admin);
				const artistBalBefore = await this.mocks.erc20.balanceOf(artist);
				const to    = this.modules.nftbid.address;
				const value = web3.utils.toWei('0');
				const data  = this.modules.nftbid.contract.methods.acceptERC1155(
					this.mocks.erc1155.address,
				...
				const adminBalAfter = await this.mocks.erc20.balanceOf(admin);
				const artistBalAfter = await this.mocks.erc20.balanceOf(artist);
			...
			after(async function () {
				...
				assert.equal(await instance.balanceOf(other2),                  		'0');
				assert.equal(await instance.balanceOf(other3),                  		'0');
				assert.equal(await this.mocks.erc1155.balanceOf(instance.address,1),'0');
				assert.equal(await this.mocks.erc1155.balanceOf(other2,1),          '1');
				assert.equal(await this.mocks.erc20.balanceOf(instance.address),       		web3.utils.toWei('9.65'));
