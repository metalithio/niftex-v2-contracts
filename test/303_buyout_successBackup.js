...
contract('Workflow', function (accounts) {
	...
	describe('Disable buyout module', function () {
		it('perform', async function () {
			await this.governance.revokeRole(await this.governance.MODULE_ROLE(), this.modules.buyout.address);
		});
	...
	describe('Claim funds', function () {
		it('default fails', async function () {
			await expectRevert(
				this.modules.buyout.claimBuyout(instance.address, { from: user1 }),
				'Access restricted to modules'
			);
		});

		it('perform', async function () {
			instance.approve(this.modules.buyout.address, web3.utils.toWei('8'), { from: user1 })
			const { receipt } = await this.modules.buyout.claimBuyoutBackup(instance.address, { from: user1 });
