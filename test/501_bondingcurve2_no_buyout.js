		...
		for (let i = 0; i < 3; i++) {
			it(`${LPAccounts[i]} withdraw ETH liquidity`, async() => {
				...
				if (ethLPTokensAmount > 0) {
					...
					const withdrawEthLiquidity = withdrawEth.logs.find(({ event }) => event === 'EtherWithdrawn').args;
					console.log('withdrawEth.gasUsed:', withdrawEth.receipt.gasUsed);
					console.log(
						`${LPAccounts[i]} withdraw ${new BigNumber(withdrawEthLiquidity.value).div(1e18).toFixed()} ETH and ${new BigNumber(withdrawEthLiquidity.payout).div(1e18).toFixed()} Shards`
					);
				} else {
		...
		for (let i = 0; i < 3; i++) {
			it(`${LPAccounts[i]} withdraw Shard liquidity`, async() => {
				...
				if (shardLPTokensAmount > 0) {
					...
					const withdrawShardLiquidity = withdrawShard.logs.find(({ event }) => event === 'ShardsWithdrawn').args;
					console.log('withdrawShard.gasUsed:', withdrawShard.receipt.gasUsed);
					console.log(
						`${LPAccounts[i]} withdraw ${new BigNumber(withdrawShardLiquidity.payout).div(1e18).toFixed()} ETH and ${new BigNumber(withdrawShardLiquidity.shards).div(1e18).toFixed()} Shards`
					);
				} else {
