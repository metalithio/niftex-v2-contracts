	...
	describe('Workflow ETH', function () {
		...
		describe('#0 Create Pool', function () {
			...
			after(async function () {
				assert.equal(await this.modules.magneticpool.getPool(this.mocks.erc721.address, 1, this.mocks.erc20.address), instance.address);
		...
		describe('#1 Deposit and create Pool', function () {
			it('perform', async function () {
				...
				const { receipt } = await this.modules.magneticpool.deposit(this.mocks.erc721.address, 1, this.mocks.erc20.address, web3.utils.toWei('8'), { from: user2 });
			});
			after(async function () {
				assert.equal(await this.modules.magneticpool.getPool(this.mocks.erc721.address, 1, this.mocks.erc20.address), instance.address);
				assert.equal(await instance.owner(),                                                                          this.modules.magneticpool.address);
		...
		describe('#2 Deposit', function () {
			it('perform', async function () {
				...
				const { receipt } = await this.modules.magneticpool.deposit(this.mocks.erc721.address, 1, this.mocks.erc20.address, web3.utils.toWei('2'), { from: user3 });
			});
			after(async function () {
				assert.equal(await this.modules.magneticpool.getPool(this.mocks.erc721.address, 1, this.mocks.erc20.address), instance.address);
				assert.equal(await instance.owner(),                                                                          this.modules.magneticpool.address);
		...
		describe('#3 Withdraw', function () {
			it('perform', async function () {
				const { receipt } = await this.modules.magneticpool.withdraw(this.mocks.erc721.address, 1, this.mocks.erc20.address, web3.utils.toWei('2'), { from: user2 });
			});
			after(async function () {
				assert.equal(await this.modules.magneticpool.getPool(this.mocks.erc721.address, 1, this.mocks.erc20.address), instance.address);
				assert.equal(await instance.owner(),                                                                          this.modules.magneticpool.address);
		...
		describe('#4 AcceptOfferERC721', function () {
			it('perform', async function () {
				...
				const { receipt } = await this.modules.magneticpool.acceptOfferERC721(this.mocks.erc721.address, 1, this.mocks.erc20.address, web3.utils.toWei('8'), { from: user1 });
			});
			after(async function () {
				assert.equal(await this.modules.magneticpool.getPool(this.mocks.erc721.address, 1, this.mocks.erc20.address), constants.ZERO_ADDRESS);
