...
contract('Workflow', function (accounts) {
	...
	before(async function () {
		...
		this.governance = await Governance.new(50400, web3.utils.toWei('0.01'), 50400, web3.utils.toWei('0.01')); // 2 weeks, 1%, 2 weeks, 1%
	...
	describe('Execute action', function () {
		it('perform', async function () {
			...
			const { receipt } = await instance.executeAction([ to ], [ data ], { from: user1 });
			expectEvent(receipt, 'ActionExecuted', { id, i: '0', to, data });
		});
