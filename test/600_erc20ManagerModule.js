...
contract('Workflow', function (accounts) {
	...
	const Modules = {
		...
		ERC20ManagerModule:     { artifact: artifacts.require('ERC20ManagerModule') },
		BasicDistributionModule:     { artifact: artifacts.require('BasicDistributionModule') },
	...
	describe('Owner try to mint erc20s via ERC20ManagerModule', function() {
		it('perform', async function () {
			const funcSig = web3.eth.abi.encodeFunctionSignature('mint(address,uint256)');
			const data = web3.eth.abi.encodeParameters(['address','uint256'],[user1, 100]);

			const calldata = funcSig + data.substring(2);

			await expectRevert.unspecified(instance.execute(this.modules.erc20managermodule.address, 0, calldata, { from: user1 }));
		})
	})

	describe('Setup basic distribution module', function () {
		it('perform', async function () {
			const { receipt } = await this.modules.basicdistributionmodule.setup(
				instance.address,
				[[ user1, 8 ], [ admin, 2 ]],
				{ from: user1 }
			);
		...
		after(async function () {
			...
			assert.equal(await instance.balanceOf(admin),                   '2');
			assert.equal(await instance.balanceOf(user3),                   '0');
			assert.equal(await instance.balanceOf(other1),                  '0');
			assert.equal(await instance.balanceOf(other2),                  '0');
			assert.equal(await instance.balanceOf(other3),                  '0');
	...
	describe('Owner try to mint erc20s via ERC20ManagerModule 2nd attempt', function() {
		it('perform', async function () {
			const funcSig = web3.eth.abi.encodeFunctionSignature('mint(address,uint256)');
			const data = web3.eth.abi.encodeParameters(['address','uint256'],[user1, 100]);
	...
	describe('Schedule action - mint erc20s via ERC20ManagerModule', function () {
		it('perform', async function () {
			const to    = this.modules.erc20managermodule.address;
			const value = '0';
			const data  = this.modules.erc20managermodule.contract.methods.mint(user1, 10).encodeABI();
	...
	describe('Wait action - mint erc20s via ERC20ManagerModule', function () {
		it('perform', async function () {
	...
	describe('Execute action - mint erc20s via ERC20ManagerModule', function () {
		it('perform', async function () {
			const to    = this.modules.erc20managermodule.address;
			const value = '0';
			const data  = this.modules.erc20managermodule.contract.methods.mint(user1, 10).encodeABI();
