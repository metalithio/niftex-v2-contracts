...
describe("MasterChef", function () {
  ...
  it("should set correct state variables", async function () {
    ...
    expect(frac).to.equal(this.frac.address)
  ...
  context("With ERC/LP token added to the field", function () {
    ...
    it("should give out FRACs only after farming time", async function () {
      ...
      expect(await this.frac.balanceOf(this.bob.address)).to.equal("0")
      ...
      expect(await this.frac.balanceOf(this.bob.address)).to.equal("1000")

      await advanceBlockTo("104")
      await this.chef.connect(this.bob).deposit(0, "0") // block 105

      expect(await this.frac.balanceOf(this.bob.address)).to.equal("5000")
      expect(await this.frac.balanceOf(this.dev.address)).to.equal("500")
      expect(await this.frac.totalSupply()).to.equal("5500")
    })
    ...
    it("should not distribute FRACs if no one deposit", async function () {
      ...
      expect(await this.frac.totalSupply()).to.equal("0")
      ...
      expect(await this.frac.totalSupply()).to.equal("0")
      expect(await this.frac.balanceOf(this.bob.address)).to.equal("0")
      expect(await this.frac.balanceOf(this.dev.address)).to.equal("0")
      ...
      expect(await this.frac.totalSupply()).to.equal("11000")
      expect(await this.frac.balanceOf(this.bob.address)).to.equal("10000")
      expect(await this.frac.balanceOf(this.dev.address)).to.equal("1000")
      expect(await this.lp.balanceOf(this.bob.address)).to.equal("1000")
    })
    ...
    it("should distribute FRACs properly for each staker", async function () {
      ...
      await this.chef.connect(this.alice).deposit(0, "10", { from: this.alice.address })
      expect(await this.frac.totalSupply()).to.equal("11000")
      expect(await this.frac.balanceOf(this.alice.address)).to.equal("5666")
      expect(await this.frac.balanceOf(this.bob.address)).to.equal("0")
      expect(await this.frac.balanceOf(this.carol.address)).to.equal("0")
      expect(await this.frac.balanceOf(this.chef.address)).to.equal("4334")
      expect(await this.frac.balanceOf(this.dev.address)).to.equal("1000")
      // Bob withdraws 5 LPs at block 330. At this point:
      //   Bob should have: 4*2/3*1000 + 2*2/6*1000 + 10*2/7*1000 = 6190
      await advanceBlockTo("329")
      await this.chef.connect(this.bob).withdraw(0, "5", { from: this.bob.address })
      expect(await this.frac.totalSupply()).to.equal("22000")
      expect(await this.frac.balanceOf(this.alice.address)).to.equal("5666")
      expect(await this.frac.balanceOf(this.bob.address)).to.equal("6190")
      expect(await this.frac.balanceOf(this.carol.address)).to.equal("0")
      expect(await this.frac.balanceOf(this.chef.address)).to.equal("8144")
      expect(await this.frac.balanceOf(this.dev.address)).to.equal("2000")
      ...
      expect(await this.frac.totalSupply()).to.equal("55000")
      expect(await this.frac.balanceOf(this.dev.address)).to.equal("5000")
      // Alice should have: 5666 + 10*2/7*1000 + 10*2/6.5*1000 = 11600
      expect(await this.frac.balanceOf(this.alice.address)).to.equal("11600")
      // Bob should have: 6190 + 10*1.5/6.5 * 1000 + 10*1.5/4.5*1000 = 11831
      expect(await this.frac.balanceOf(this.bob.address)).to.equal("11831")
      // Carol should have: 2*3/6*1000 + 10*3/7*1000 + 10*3/6.5*1000 + 10*3/4.5*1000 + 10*1000 = 26568
      expect(await this.frac.balanceOf(this.carol.address)).to.equal("26568")
    ...
    it("should give proper FRACs allocation to each pool", async function () {
      ...
      expect(await this.chef.pendingFrac(0, this.alice.address)).to.equal("10000")
      // Bob deposits 10 LP2s at block 425
      await advanceBlockTo("424")
      await this.chef.connect(this.bob).deposit(1, "5", { from: this.bob.address })
      // Alice should have 10000 + 5*1/3*1000 = 11666 pending reward
      expect(await this.chef.pendingFrac(0, this.alice.address)).to.equal("11666")
      await advanceBlockTo("430")
      // At block 430. Bob should get 5*2/3*1000 = 3333. Alice should get ~1666 more.
      expect(await this.chef.pendingFrac(0, this.alice.address)).to.equal("13333")
      expect(await this.chef.pendingFrac(1, this.bob.address)).to.equal("3333")
    })
    ...
    it("should stop giving bonus FRACs after the bonus period ends", async function () {
      ...
      expect(await this.chef.pendingFrac(0, this.alice.address)).to.equal("10500")
      // At block 606, Alice withdraws all pending rewards and should get 10600.
      await this.chef.connect(this.alice).deposit(0, "0", { from: this.alice.address })
      expect(await this.chef.pendingFrac(0, this.alice.address)).to.equal("0")
      expect(await this.frac.balanceOf(this.alice.address)).to.equal("10600")
    })
  })
})
